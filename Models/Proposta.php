    ...
    public function listPropId($id,$status){
        ...
        try{
            ...
            if ($sql->rowCount() > 0){
                ...
                while($conteudo = $sql->fetch(\PDO::FETCH_ASSOC)){
                    if($status == 1){
                        $array['dados'] .= '<tr><td>'.$conteudo['data_proposta'].'</td><td>'.$conteudo['operacao'].'</td><td>'.$conteudo['nome_cli'].'</td><td>'.$conteudo['valor'].'</td><td>'.$conteudo['nome_colaborador'].'</td><td>'.$conteudo['data_atualizacao'].'</td><td><div class="btn-group"><a href="" class="btn btn-xs btn-primary">Editar</a><button id="'.$conteudo['id'].'" class="desativadas btn btn-xs btn-danger">Desativar</button><button id="'.$conteudo['id'].'" class="btn btn-xs btn-primary">Relatorio</button><button id="'.$conteudo['id'].'" class="analise btn btn-xs btn-danger" target="_blank">Analise</button><button id="'.$conteudo['id'].'" class="canceladas btn btn-xs btn-danger" target="_blank">Cancelar</button></div></td><tr>';
                    }
                    if($status == 2){
